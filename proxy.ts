...
/**
 * 必须与 page-agent 官方 SYSTEM_PROMPT 完全一致
 * 这里按你上面给的 JS 原文整理
 */
const OFFICIAL_PAGE_AGENT_SYSTEM_PROMPT = `You are an AI agent designed to operate in an iterative loop to automate browser tasks. Your ultimate goal is accomplishing the task provided in <user_request>.
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const firstMessage = body.messages[0] as ChatMessage | undefined;
    if (!firstMessage) {
      return invalidRequest("messages[0] is required");
    }

    if (
      firstMessage.role !== "system" &&
      firstMessage.role !== "developer"
    ) {
      return invalidRequest(
        "First message must have role='system' or role='developer'. This proxy is for page-agent.",
      );
    }

    const firstContent = normalizeContent(firstMessage.content);
    if (firstContent !== OFFICIAL_PAGE_AGENT_SYSTEM_PROMPT) {
      return invalidRequest(
        "System prompt must match the official page-agent system prompt. This proxy is for page-agent.",
      );
    }
    ...
    if (body.parallel_tool_calls !== undefined) {
      upstreamPayload.parallel_tool_calls = body.parallel_tool_calls;
    }
    ...
    if (body.reasoning_effort !== undefined) {
      upstreamPayload.reasoning_effort = body.reasoning_effort;
    }

    if (body.verbosity !== undefined) {
      upstreamPayload.verbosity = body.verbosity;
    }
    ...
    if (body.enable_thinking !== undefined) {
      upstreamPayload.enable_thinking = body.enable_thinking;
    }

    if (body.reasoning !== undefined) {
      upstreamPayload.reasoning = body.reasoning;
    }

    const upstreamRes = await fetch(`${UPSTREAM_BASE_URL}${UPSTREAM_PATH}`, {
      method: "POST",
      headers: {
        ...
        "user-agent": "Deno/2.x moehans-page-agent-proxy",
      },
